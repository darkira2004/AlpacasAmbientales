class SidebarController {    constructor() {
        ...
        // Establecer item activo basado en la página actual
        setTimeout(() => {
            this.setActiveBasedOnCurrentPage();
        }, 100); // Pequeño delay para asegurar que el sidebar esté completamente cargado
    }
    ...
    setupEventListeners() {
        ...
        if (sidebarToggle) {
            console.log(`🍔 Botón hamburguesa encontrado: #${sidebarToggle.id}`);
            
            // Listener principal 
            sidebarToggle.addEventListener('click', (e) => {
                e.preventDefault();
                console.log('🍔 Click en botón hamburguesa desde SidebarController');
                this.toggle();
            });
            
            console.log('🔧 Listener configurado para botón hamburguesa en SidebarController');
        } else {
            console.error('❌ Botón hamburguesa NO encontrado (buscó #open-sidebar y #sidebar-toggle)');
        }
    ...
    /**
     * Método para marcar un item del sidebar como activo
     * @param {string} itemId - ID del item a marcar como activo
     */
    setActiveMenuItem(itemId) {
        console.log(`🎯 Marcando item como activo: ${itemId}`);
        ...
        // Marcar el item específico como activo
        ...
        if (targetItem) {
            ...
            console.log(`✅ Item ${itemId} marcado como activo`);
        } else {
            console.warn(`⚠️ Item ${itemId} no encontrado en el sidebar`);
        }
    }

    /**
     * Método para determinar el item activo basado en la URL actual
     */
    setActiveBasedOnCurrentPage() {
        ...
        // Mapeo de páginas a items del sidebar
        const pageMapping = {
        ...
        if (activeItemId) {
            ...
            console.log(`📄 No hay mapeo definido para: ${currentFile}`);
        }
    }
}
...
// Inicializar cuando el DOM esté listo - COMENTADO para evitar conflictos
/*
document.addEventListener('DOMContentLoaded', () => {
    if (!window.sidebarControllerInstance) {
        window.sidebarControllerInstance = new SidebarController();
        console.log('✅ SidebarController instancia global creada');
    }
});
*/